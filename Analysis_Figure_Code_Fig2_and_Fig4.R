...
exp = read.csv("all.exps.survival.csv")
...
#6 control.moderate 0.6072222
 ...
 exp.recruit = read.csv("all.exps.recruit.csv")
 ...
 survey = read.csv("reef_survey_summary.csv")
...
# Figures 2a and 2b for field experiment ####
...
levels(exp.recruit$complexity)[levels(exp.recruit$complexity)=="high"] <- "High"
 ...
 #Figure 4 for Field Survey ####
 fig4a = ggplot(survey, aes(sample.wt, log.adults)) +
   geom_point(size = 3) +
 ...
 fig4b = ggplot(survey, aes(sample.wt, log.spat)) +
   geom_point(size = 3) +
   stat_smooth(method = "lm", formula = y ~ x + I(x^2), color = 'blue', size = 1) +
   ylim(0,5) +
   xlim(0,11) +
 ...
 grid.arrange(fig4a,fig4b, ncol = 1)
